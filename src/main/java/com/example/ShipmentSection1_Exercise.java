...
//  ชื่อนักศึกษา : ธนภูมิ แทนทุมมา
//  รหัสนักศึกษา : 673380271-3
// ╚══════════════════════════════════════════════════════════╝
//
//  โจทย์:
...
class Shipment {
    ...
    public String toString() {
    	String BaseString = "[%s] %.2f กก. | %s |	%.2f";
    	String ReturnString;
    	ReturnString = String.format(BaseString, getTrackingNumber(), getWeightKg(), getType(), calculateCost());
        return ReturnString;  // ← เติมให้ครบ
    }
}
...
class ShippingCompany {
    ...
    public double getTotalCost() {
        ...
        for (Shipment Each : shipments) {
        	total += Each.calculateCost();
        }
        return total;
    }
